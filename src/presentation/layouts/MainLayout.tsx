...
import { Bell, ChartNoAxesCombined, CircleHelp, CreditCard, LayoutDashboard, ShieldCheck, Target, WalletCards } from 'lucide-react'
...
export function MainLayout() {
  return (
    <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[252px_1fr]">
      ...
      <div className="min-w-0"><header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6 lg:px-8"><div className="flex items-center gap-2 lg:hidden"><ShieldCheck className="text-emerald-600" size={21} /><span className="font-bold text-slate-950">COFRE IA</span></div><div className="hidden text-sm text-slate-500 lg:block">Sua central financeira pessoal</div><div className="flex items-center gap-3"><button type="button" aria-label="Notificações" className="relative rounded-lg p-2 text-slate-500 transition hover:bg-slate-100"><Bell size={20} /><span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-emerald-500 ring-2 ring-white" /></button><div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-950 text-sm font-bold text-white">J</div></div></header><main><Outlet /></main><footer className="px-6 pb-6 text-center text-xs text-slate-400 lg:hidden"><CircleHelp className="mr-1 inline" size={13} />Seu dinheiro, com mais clareza.</footer></div>
    </div>
  )
}
