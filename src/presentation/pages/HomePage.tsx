...
export function HomePage() {
  ...
  const displayName = useMemo(() => {
    const rawName = user?.name?.trim() || user?.email?.split('@')[0]?.trim() || 'Usuário'
  ...
  return (
    ...
    >
      <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-1 text-sm font-medium text-emerald-600">Sexta-feira, 15 de agosto</p><h1 className="text-3xl font-bold tracking-tight text-slate-950">{greeting}, {displayName}</h1><p className="mt-1 text-slate-500">Seu dinheiro está organizado. Vamos olhar o que importa hoje?</p></div><button type="button" onClick={() => setShowForm((current) => !current)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800"><Plus size={18} /> Adicionar movimentação</button></div>
