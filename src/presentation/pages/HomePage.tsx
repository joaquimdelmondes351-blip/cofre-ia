...
export function HomePage() {
  ...
  return (
    ...
    >
      <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-1 text-sm font-medium text-emerald-600">Sexta-feira, 15 de agosto</p><h1 className="text-3xl font-bold tracking-tight text-slate-950">Bom dia, Joaquim</h1><p className="mt-1 text-slate-500">Seu dinheiro está organizado. Vamos olhar o que importa hoje?</p></div><button type="button" onClick={() => setShowForm((current) => !current)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800"><Plus size={18} /> Adicionar movimentação</button></div>
      {showForm && <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} onSubmit={addTransaction} className="mb-6 grid gap-3 overflow-hidden rounded-2xl border border-emerald-100 bg-emerald-50 p-4 md:grid-cols-[1.4fr_1fr_1fr_1fr_auto]"><input name="name" required placeholder="Ex.: Mercado" className="rounded-lg border border-emerald-100 bg-white px-3 py-2.5 text-sm outline-none ring-emerald-400 focus:ring-2" /><input name="amount" required min="0.01" step="0.01" type="number" placeholder="Valor (R$)" className="rounded-lg border border-emerald-100 bg-white px-3 py-2.5 text-sm outline-none ring-emerald-400 focus:ring-2" /><select name="category" className="rounded-lg border border-emerald-100 bg-white px-3 py-2.5 text-sm">{categories.map((category) => <option key={category}>{category}</option>)}</select><div className="flex rounded-lg border border-emerald-100 bg-white p-1 text-sm font-medium"><button type="button" onClick={() => setTransactionType('expense')} className={`flex-1 rounded-md px-2 ${transactionType === 'expense' ? 'bg-rose-50 text-rose-600' : 'text-slate-500'}`}>Despesa</button><button type="button" onClick={() => setTransactionType('income')} className={`flex-1 rounded-md px-2 ${transactionType === 'income' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500'}`}>Receita</button></div>{transactionType === 'expense' && <div className="flex gap-2 rounded-lg border border-emerald-100 bg-white p-1 text-sm font-medium"><select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value as 'cash' | 'card')} className="flex-1 rounded-md bg-transparent px-2 outline-none"><option value="cash">Dinheiro/conta</option><option value="card">Cartão</option></select>{paymentMethod === 'card' && cards.length > 0 && <select value={selectedCardId} onChange={(event) => setSelectedCardId(event.target.value)} className="flex-1 rounded-md bg-transparent px-2 outline-none"><option value="">Selecione o cartão</option>{cards.map((card) => <option key={card.id} value={card.id}>{card.name}</option>)}</select>}</div>}{transactionType === 'expense' && paymentMethod === 'card' && cards.length === 0 && <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">Cadastre um cartão antes de registrar uma despesa com cartão.</div>}<button className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Salvar</button></motion.form>}
